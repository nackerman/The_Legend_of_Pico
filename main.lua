 ...
 game_state="game_start"
	...
	dir=
	{
	 --definitions
		...
		--priority direction:
		--gets updated with time stamp
		--on frame dir btn is last
		--pressed
		priority={
			["⬅️"]=-1,
			["➡️"]=-1,
...
function draw_sp_mask(
sp,dx,dy,f,mask_color)
	--sp 			= sprite number
	--f					= sp flipped?
...
function draw_active_bomb()
	if bomb_active then
		if bomb_timer
		<=bomb_timer_start/5 then
			spr(233+global_timer%2,
				bomb_coord.x,
				bomb_coord.y
			)
		elseif bomb_timer
		<=(bomb_timer_start/5)*2 then
			spr(231+global_timer%2,
				bomb_coord.x,
				bomb_coord.y
			)
		elseif bomb_timer
		<=(bomb_timer_start/5)*3 then
			spr(229+global_timer%2,
				bomb_coord.x,
				bomb_coord.y
			)
		elseif bomb_timer
		<=(bomb_timer_start/5)*4 then
			spr(227+global_timer%2,
				bomb_coord.x,
				bomb_coord.y
			)
		else
			spr(225+global_timer%2,
				bomb_coord.x,
				bomb_coord.y
			)
		end
...
function player_move()
	...
	--soft edge variables
	local soft_edge_px=6 --not>6
	local soft_edge_spd=1
	
	--calculate distance to
	--nearest edge. if sprite is
	--perfectly within a tile,
	...
	if not p.att then
		...
		if d=="⬅️" then
			...
			if left_ok then
				...
				--clear top edge
				if dist_top<=soft_edge_px
				and dist_top>0
				and not is_blocked_at(
				p.x-1,p.y) then
					--nudge up to clear edge
					p.y-=1
				end
				--clear bottom edge
				if dist_bot<=soft_edge_px
				and dist_bot>0
				and not is_blocked_at(
				p.x-1,p.y+7) then
...
function can_move()
	...
	if not fget(tile_sp_1,0) and
 not	fget(tile_sp_2,0) then
 	result.right=true
 else
 	result.right=false
	end
...
function player_collision()
	...
	for e in all(enemies) do
		...
		if box~=nil then
			...
			if p.att_hb~=nil 
			and p.att_count==p.att_time  
			then
				--att_hb exists, and first
   	--frame of attack
...
function collect_item()
	...
	for i in all(items) do
		...
		if i.id=="rp_g" or
			...
			if collision(p.hb,box) then
				if p.rupees+rupee_val[i.id]
				<= p.max_rupees then
					sfx(2)
					p.rupees+=rupee_val[i.id]
					del(items,i)
				elseif p.rupees~=p.max_rupees
				and p.rupees+rupee_val[i.id]
				>=p.max_rupees then
					sfx(2)
...
function enemy_in_bomb_r(ehb)
	--ehb=enemy hitbox
	if bomb_explode_time
	==bomb_explode_time_start-5 then
		return in_circle(
			ehb.x1,
			ehb.y1,
			bomb_coord.x,
...
function bomb_sfx()
	if bomb_explode_time
	==bomb_explode_time_start-1 then
		sfx(8)
	end
end
